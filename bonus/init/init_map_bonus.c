...
/*   Updated: 2024/09/10 15:38:52 by alexandra        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @brief Processes a single line of the map file to determine if it contains map data.
 *
 * This function checks if the line contains valid map content (starts with '1' or '0') 
 * or if it contains other configuration
 * lines like texture paths or color definitions. If it finds the start of the map, it 
 * sets the appropriate flags in the data 
 ...
 *
 * @return 0 if successful, 1 if there is an error in processing the configuration lines.
 */
...
/**
 * @brief Scans the map file to locate and process the map data, counting its dimensions.
 *
 * This function iterates through each line of the map file, looking for the start of the map.
 ...
 * 
 * @return A double pointer to the buffer containing the map lines, or NULL on error.
 */
...
/**
 * @brief Initializes the map by loading and parsing the .cub file for the bonus version of cub3D.
 *
 ...
 * The function also sets the ceiling and floor colors based on parsed RGB values.
